...
@inject
class BoxService:
    def __init__(self, repository : BoxRepository, lock : BoxLock, user_subject : UserSubject, locker_subject: LockerSubject):
    ...
    def retrieve_package(self, id, locker_id, verification):
        ...
        if verify == True:
            search_trans = self.repository.search_incomplete_transaction_by_locker_id(locker_id)
            cur_time = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
            self.repository.complete_transaction(id, locker_id, search_trans[0], cur_time)
            return True
        else:
            return verify
        #END#
    ...
    def store_package(self, id, item_type, item_size):
        ...
        verify_trans_id = self.repository.search_trans_by_trans_id(trans_id)    # Check if trans_id has been used or not
        ...
        self.repository.store_package_update(trans_id, id, item_type, item_size, verify, cur_time, verification)    # Insert new transaction data

        ### Send emails
        search_user_details = self.repository.search_by_id(id)

        self.notify_new_package_to_user(search_user_details[2], search_user_details[1], verify, item_type, item_size, verification)
    ...
    def notify_new_package_to_user(self, email, name, locker_id, item_type, item_size, verification):
        self.locker_subject.notify(NewPackageMessage(email, name, locker_id, item_type, item_size, verification))
    #END#
    ...
    def remind_users_to_retrieve_package(self):
        search_incomplete_transactions = self.repository.search_user_name_and_email_with_incomplete_transactions()
        self.locker_subject.notify(RetrievePackageReminderMessage(search_incomplete_transactions))
    #END#
    ...
    def retrieve_package_verify(self, id, locker_id, verification):
        ...
        search_trans = self.repository.search_incomplete_transaction_by_locker_id(locker_id)
        search_user = self.repository.search_by_id(id)
        search_trans_detail = self.repository.search_trans_detail_by_locker_id(locker_id, search_trans[0])
