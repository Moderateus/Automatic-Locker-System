...
class BoxLock(Lock):
    ...
    def lock(self, locker_id):
        ...
        ### This is for locker number 5 to 8, where we use alphabet A until D for locking lockers number 5 to 8
            ...
            time.sleep(2)
            self.ser.write(bytes(chr(((locker_id - (locker_id - 5)) * 13) + (locker_id % 5)), encoding='utf-8'))
    ...
    def unlock(self, locker_id):
        if 1 <=  locker_id <= 4:
            ...
            time.sleep(2)
            self.ser.write(bytes(chr(((locker_id - (locker_id - 5)) * 13) + (locker_id % 5) + 4), encoding='utf-8'))
